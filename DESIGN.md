...
- Use **Landometer Design System 0.9.4**, pinned authoring `0.9.4-r2`, ruleset `lds-rules-0.9.4`, machine package `v0.9.4-mp1`. [DS asset integration](DS_ASSET_INTEGRATION.md) explains usage; [the asset manifest](contracts/ds-assets.v1.2.json) gives exact paths, roles and SHA-256 hashes.
...
- Use registered motifs only for their allowed decorative orientation/transition role. A motif is not a logo, map mark, loading state or analytical value.
...
Keep Demand and Supply as distinct visual and conceptual groups. The baseline fuel criteria are the three building and six activity metrics specified in the [product statement](CityMETER_Yolk_Product_Brief_v1.2.md). A high-demand flag is an evidence-based **proxy**, not measured visits or sales. A branch count is not market share or capacity. An unverified POI stays unverified until reviewed.
