...
def verify(root, zip_path=None):
    ...
    contracts = ["contracts/assets.v1.5.json", "contracts/ds-assets.v1.5.json", "contracts/icons.v1.5.json", "contracts/web-identity.v1.5.json"]
    assets = set()
    for name in contracts:
        require((root / name).is_file(), "Required release asset contract missing: " + name)
        assets.update(verify_declared_assets(root, name))
    release_path = root / "contracts/release.v1.5.0.json"
    ...
    runtime_paths = verify_declared_assets(root, "contracts/release.v1.5.0.json")
    ...
    asset_contract = read_json(root / "contracts/assets.v1.5.json")
