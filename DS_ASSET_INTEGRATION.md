...
**For dev:** use this with [the machine manifest](contracts/ds-assets.v1.3.json), [the implementation plan](IMPLEMENTATION_PLAN_v1.3.md), and [the DS resolution](evidence/ds-resolution.md). All paths in the manifest are relative to this package root. The prototype is a multi-file static browser preview; its `prototype/index.html` loads assets by relative URL, so it can sit below a repository base path such as `/yolk/prototype/`.
...
5. **Place identity directly; omit motifs.** Place the unchanged transparent logo directly on the existing light page surface. Do not create a frame, card, badge or backing plate. Dark mode uses the approved governed live-text identity `Landometer` with the exact Arvo 700 / 18px / 1.2 / zero-tracking app-header binding because the pinned package has no inverse raster lockup. Do not recolour or crop the logo. All motifs are disabled and excluded from Yolk runtime, following the owner’s explicit correction. Use DS-style interface icons only where they clarify a control, alongside visible labels. See [asset index](ASSET_INDEX_v1.3.md).
...
See [the experience contract](contracts/experience.v1.3.json) and [ordered implementation steps](docs/EXPERIENCE_v1.3.md). The active `ds-assets.v1.3.json` pins the selected immutable DS assets; `icons.v1.3.json` pins the product icon extension. The older v1.2 inventory is historical only. Current product adapter hashes belong in [the release contract](contracts/release.v1.3.1.json). Release status is `ready_with_open_manual_gate`: source token/contrast checks are not rendered visual verification.
...
m = json.loads(pathlib.Path('contracts/ds-assets.v1.3.json').read_text())
for a in m['assets']:
